...
func (k msgServer) letsPayToExperts(ctx sdk.Context, id uint64, correctAnswer int, loserPool *big.Int, tokenMinted *big.Int) (bool, string) {
	var percent int
	if len(k.GetAdvisorPubEvent(ctx, id)) > 2 {
	...
	if allReputation > 0 {
		...
		for i := 0; i < len(valid); i++ {
			...
			if reputation >= 0 {
				if k.canMint(ctx, id) {
					// mint tokens
					mint := ExpertFormula(tokenMinted, new(big.Int).SetInt64(int64(FEtypes.ExpertPercMint)), new(big.Int).SetInt64(int64(reputation)), new(big.Int).SetInt64(int64(allReputation)))
					fmt.Printf("mint %s, wallet %s", mint.String(), wallet)
					ok, err := k.letsMint(ctx, wallet, mint)
					if !ok {
						return false, err
					}
				}
				...
				am := CalcPayForExpert(new(big.Int).SetInt64(int64(percent)), loserPool, new(big.Int).SetInt64(int64(reputation)), new(big.Int).SetInt64(int64(allReputation)), premAmount, new(big.Int).SetInt64((int64(FEtypes.ExpertPremiumPerc))))
				fmt.Printf("amount pay %s, wallet %s", am.String(), wallet)
				ok, err := k.LetsPay(ctx, wallet, am)
				if !ok {
					return false, err
				}
			}
...
func (k msgServer) letsPayToPlayers(ctx sdk.Context, id uint64, correctAnswer int, loserPool *big.Int, mintedToken *big.Int) (bool, string, *big.Int, *big.Int) {
	...
	for i := 0; i < rightPlayLenght; i++ {
		...
		// pay token
		am := CalcPlayerPay(premAmount, winPool, uB, avarageBetWin, premimWin)
		ok, err := k.LetsPay(ctx, wallet, am)
		if !ok {
			return ok, err, zero, zero
		}
...
func (k msgServer) letsPayWithFormula(tokenMinted *big.Int, ctx sdk.Context, addr string, percent int) (bool, string) {
	...
	mintDF := GetPercent(new(big.Int).SetInt64(int64(percent)), tokenMinted)
